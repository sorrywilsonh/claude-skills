...
| Brand site URL | `${SKILL_DIR}/.venv/bin/python skills/ppt-master/scripts/source_to_md/web_to_md.py <URL>`, then `Read` the result | color references, font references, voice/tone |
| Branded PPTX | `${SKILL_DIR}/.venv/bin/python skills/ppt-master/scripts/source_to_md/ppt_to_md.py <file>`, then read theme XML | colors, typography (literal) |
| Brand PDF | `${SKILL_DIR}/.venv/bin/python skills/ppt-master/scripts/source_to_md/pdf_to_md.py <file>` | voice/tone; sometimes color/font references |
...
- To edit: modify `templates/brands/<brand_id>/design_spec.md` directly, then re-run `${SKILL_DIR}/.venv/bin/python skills/ppt-master/scripts/register_template.py --kind brand <brand_id>`
```
