...
> **⚙️ LOCAL SETUP (this machine):** Python deps are installed in a dedicated venv at `${SKILL_DIR}/.venv` (Python 3.12, via uv). The system `python3` is 3.9 and will NOT work — every command in this file already points at `${SKILL_DIR}/.venv/bin/python`; always use that interpreter, never bare `python3`. Installed: `python-pptx`, `Pillow`, `numpy`, `lxml`. NOT installed (need Homebrew + cairo): SVG→PNG rasterizers (`cairosvg`/`svglib`) — so export with `--no-compat` (pure native SVG; renders in modern PowerPoint/Keynote). AI image generation (`image_gen.py`) needs an `.env` with a provider key (e.g. `OPENAI_API_KEY`); without it, skip AI images or use `image_search.py` (Openverse/Wikimedia, no key).
...
|---------------|---------|
| PDF file | `${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/source_to_md/pdf_to_md.py <file>` |
| DOCX / Word / Office document | `${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/source_to_md/doc_to_md.py <file>` |
| XLSX / XLSM / Excel workbook | `${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/source_to_md/excel_to_md.py <file>` |
| CSV / TSV | Read directly as plain-text table source |
| PPTX / PowerPoint deck | `${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/source_to_md/ppt_to_md.py <file>` |
| EPUB / HTML / LaTeX / RST / other | `${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/source_to_md/doc_to_md.py <file>` |
| Web link | `${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/source_to_md/web_to_md.py <URL>` |
| WeChat / high-security site | `${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/source_to_md/web_to_md.py <URL>` (requires `curl_cffi`, included in `requirements.txt`) |
...
```bash
${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/project_manager.py init <project_name> --format <format>
```
...
| Situation | Action |
|-----------|--------|
| Has source files (PDF/MD/etc.) | `${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/project_manager.py import-sources <project_path> <source_files...> --move` |
...
3. Run:
   ```bash
   ${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/latex_render.py <project_path>
   ```
...
```bash
${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/analyze_images.py <project_path>/images
```
...
|---|---|---|
| `ai` | `references/image-generator.md` | `${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/image_gen.py --manifest <project_path>/images/image_prompts.json` |
| `web` | `references/image-searcher.md` | `${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/image_search.py ...` |
...
```bash
${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/svg_editor/server.py <project_path> --live
```
...
```bash
${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/svg_quality_checker.py <project_path>
```
...
```bash
${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/total_md_split.py <project_path>
```

**Step 7.2** — SVG post-processing (icon embedding / image crop & embed / text flattening / rounded rect to path):
```bash
${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/finalize_svg.py <project_path>
```

**Step 7.3** — Export PPTX (embeds speaker notes by default):
```bash
${SKILL_DIR}/.venv/bin/python ${SKILL_DIR}/scripts/svg_to_pptx.py <project_path>
...
- Local preview: `${SKILL_DIR}/.venv/bin/python -m http.server -d <project_path>/svg_final 8000`
